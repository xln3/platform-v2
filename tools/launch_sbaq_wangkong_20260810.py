"""盛邦网空线 16 题正式采集发起（20260810）。

两套冻结配置（「各问一次」口径）：
- A: doubao/yiyan × normal × 北京/上海 × 16 题 = 64 任务
- B: deepseek × deep_think × 北京/上海 × 16 题 = 32 任务（深度思考开=思考过程采集）
...
def freeze(client: httpx.Client, tag: str, models: list[str], modes: list[str]) -> str:
    body = {
        "query_groups": [
            {
                "name": GROUP_NAME,
    ...
    resp = client.post(
        ...
        headers={"Idempotency-Key": f"sbaq-wangkong-{tag}-20260810"},
        json=body,
    )
...
def launch(client: httpx.Client, tag: str, config_pub_id: str) -> dict:
    resp = client.post(
        "/api/v2/collection/runs",
        headers={"Idempotency-Key": f"sbaq-wangkong-run-{tag}-20260810"},
        json={
...
def main() -> None:
    ...
    with httpx.Client(
        ...
        cfv_a = freeze(client, "cfgA-doubao-yiyan", ["doubao", "yiyan"], ["normal"])
        print(json.dumps({"cfgA": cfv_a}, ensure_ascii=False))
        cfv_b = freeze(client, "cfgB-deepseek-think", ["deepseek"], ["deep_think"])
        print(json.dumps({"cfgB": cfv_b}, ensure_ascii=False))

        run_a = launch(client, "A", cfv_a)
        print(json.dumps({"runA": run_a}, ensure_ascii=False))
        run_b = launch(client, "B", cfv_b)
