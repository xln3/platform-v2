...
SERVICE_CATALOG: tuple[ServiceDefinition, ...] = (
    ...
    ServiceDefinition(
        ...
        unit="项",
        summary=(
            "核查约定运行、问答和时间窗内可观测到的全部 U 信源帖子，逐帖识别谁评价谁、"
            "无锚降位、操纵比较和误导性遗漏；归属证据独立记录，不作为入池条件。"
        ),
        scope=(
            "枚举冻结范围内全部 U occurrence；同一 URL 可复用抓取，但保留每条回答与查询暴露",
            "识别实体方向、事实性负面、无锚贬评、次级放置、操纵比较和关键事实遗漏",
            "对每条候选保留不可变页面、逐字原文、上下文、视觉锚点和独立事实核查依据",
        ),
        inputs=(
            "纳入核查的采集运行、问答范围、时间窗与 U snapshot boundary",
            "品牌、产品、竞品和其他实体别名；名单仅辅助识别，不用于预先筛帖",
            "客户确认的产品事实、资质与可选发布/委托归属证据",
        ),
        outputs=(
            "全 U 覆盖漏斗、实体—关系 finding 与版本化风险分级",
            "逐字原文、页面 hash、URL、位置与视觉证据",
            "独立事实核查及 publisher/commissioner 归属置信度；证据不足时保持 unknown",
        ),
    ),
    ServiceDefinition(
        number=3,
...
PACKAGE_CATALOG: tuple[PackageDefinition, ...] = (
    PackageDefinition(
        ...
        preconditions=(
            ...
            "服务 2 冻结运行、问答、时间窗和判据版本，并核查该范围内全部 U occurrence。",
        ),
