...
def test_inline_decision_request_does_not_enqueue_a_second_workflow_owner() -> None:
    token = uuid4().hex
    artifacts = build_seed_bundle()
    seed(POSTGRES_DSN, artifacts)
    task = next(item for item in artifacts if item.kind == "decision_task")
    ...
    requested = repository.create_decision_request(
        tenant_pub_id=tenant,
        ...
        context_hash=digest(f"context:{token}"),
        judge_policy_hash=policy.content_hash,
        idempotency_key=f"inline-decision-{token}",
    ...
    with psycopg.connect(POSTGRES_DSN) as connection:
        command_count = connection.execute(
            ...
            """,
            (tenant, f"decision-v2:{requested['decision_job_pub_id']}"),
        ).fetchone()
        outbox_count = connection.execute(
            """
            SELECT count(*) FROM integration.outbox_event
            WHERE tenant_pub_id=%s AND aggregate_pub_id=%s
              AND event_type='semantic.decision.requested.v2'
            """,
            (tenant, requested["decision_job_pub_id"]),
        ).fetchone()
