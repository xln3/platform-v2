...
class _FakePage:
    ...
    def __init__(self, *, captcha_visible: bool = False,
                 jpeg: bytes = _FAKE_JPEG, iframe_offset: dict | None = None) -> None:
...
def _wire(
    ...
    *, cdp_url: str | None = "http://127.0.0.1:19222",
    ...
    pushes: list[dict] = []
    monkeypatch.setattr(
        captcha_assist, "push_captcha_assist",
...
def test_registry_file_contract(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    record = {
        "version": 1, "run_pub_id": "run_1", "session_id": "s" * 24,
        "ticket_hash": th, "port": 12345, "platform": "doubao", "state": "active",
        "business_key": "bk", "evidence_ref": None,
        "created_at": 1786000000, "expires_at": 1786004200,
        "push_sent": True, "solved_at": None,
    }
...
async def test_start_idempotent_and_stop_closes(
    ...
    assert (r2.session_id, r2.assist_url, r2.pushed) == (
        r1.session_id, r1.assist_url, r1.pushed)
    ...
    await captcha_assist_stop(CaptchaAssistStopInput(
        run_pub_id="run_1", session_id=r1.session_id))
    ...
    await captcha_assist_stop(CaptchaAssistStopInput(
        run_pub_id="run_1", session_id=r1.session_id))
    await captcha_assist_stop(CaptchaAssistStopInput(
        run_pub_id="run_ghost", session_id="nonexistent"))


async def test_cdp_url_missing_non_retryable(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
...
async def test_ttl_suicide_closes_session(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    page = _FakePage(captcha_visible=True)
...
async def test_page_selection_prefers_captcha_hit(
    ...
    body, content_type = await asyncio.to_thread(
        _http_get, f"http://127.0.0.1:{rec['port']}/frame")
