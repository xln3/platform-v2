...
def _answer(pub_id: str, *, model: str = "doubao", region: str = "北京",
            query: str = "保险公司推荐") -> dict:
    return {"pub_id": pub_id, "query_text": query, "response_text": f"文本{pub_id}",
            "model": model, "region": region, "mode": "normal", "capture_time": NOW}


def _ok_row(brands: list[str]) -> dict:
    return {"brands": brands, "status": "ok", "model": "m-fanout", "error": None,
            "domain": "insurance", "extracted_at": NOW.isoformat()}


def _project(brandrank_domain: str | None = "insurance",
             brand_names: list[str] | None = None,
             competitor_names: list[str] | None = None) -> dict:
...
def _env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    monkeypatch.setattr(extract, "default_client",
                        lambda: pytest.fail("报告事实路径严禁调用 LLM"))
    yield
...
def _patch_fetch(monkeypatch: pytest.MonkeyPatch, *, answers: list[dict],
                 project: dict | None, table_rows: dict[str, dict] | None = None) -> dict:
    ...
    def fake_fetch_answers(dsn: str, tenant_pub_id: str, project_pub_id: str,
                           since: datetime):
        seen["since"] = since
...
def _row(body: dict, *, platform: str, region: str, query: str,
         metric: str) -> dict[str, Any]:
    for row in body["fact_rows"]:
        dims = row["dimensions"]
        if (dims["platform"], dims["region"], dims["query"], row["metric"]) == (
                platform, region, query, metric):
            return row
...
def test_no_extraction_coverage_insufficient(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert body["fact_rows"] == []
    assert body["coverage"] == {"n_answers": 3, "n_with_extract": 0,
                                "n_groups": 0, "n_fact_rows": 0}
...
def test_target_brand_unset_insufficient(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _override_principal()
    _patch_fetch(monkeypatch, answers=[_answer("ans_1")],
...
def test_four_metrics_mapping_against_metrics_module(
    ...
    assert body["coverage"] == {"n_answers": 5, "n_with_extract": 5,
                                "n_groups": 3, "n_fact_rows": 18}
    ...
    group_records = {
        ...
        ("doubao", "北京", "q2"): [{"brands": ["中意人寿"], "query": "q2"},
                                   {"brands": [], "query": "q2"}],
    }
    expected = {
        key: metrics.brand_special(records, "中意人寿", rules=rules,
                                   total_count=len(records), top_ns=(1, 3, 5))
    ...
    for (platform, region, query), special in expected.items():
        ...
        # ① 品牌提及率
        row = _row(body, platform=platform, region=region, query=query,
        ...
        comp = metrics.brand_special(group_records[(platform, region, query)], "中国平安",
                                     rules=rules, total_count=n, top_ns=(1, 3, 5))
...
def test_uncovered_answers_excluded_from_denominator(
        monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    row = _row(body, platform="doubao", region="北京", query="保险公司推荐",
               metric="brand_appearance_rate")
...
def test_response_has_no_account_or_profile_dimensions(
    ...
    for forbidden in ("platform_account", "browser_profile", "session_event",
                      "authorization_scope", "response_text", "文本ans_"):
