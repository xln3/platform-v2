...
def _seams(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_extracts(
        dsn: str, tenant: str, ids: list[str], domain: str
...
def test_before_after_none_when_params_partial() -> None:
    assert (
        _compute(before_start="2026-07-01", before_end="2026-07-07", after_start="2026-08-01")[
            "before_after"
        ]
        is None
    )
...
def test_before_after_none_when_params_malformed_or_reversed() -> None:
    ...
    assert (
        _compute(
            before_start="2026-07-07",
            before_end="2026-07-01",
            after_start="2026-08-01",
            after_end="2026-08-07",
        )["before_after"]
        is None
...
def test_before_after_insufficient_when_arm_no_coverage(monkeypatch: pytest.MonkeyPatch) -> None:
    # after 臂答案在、但抽取表零覆盖（failed/缺失）→ 诚实占位
    monkeypatch.setattr(
        brandrank_service,
        "fetch_brand_extracts",
        lambda dsn, tenant, ids, domain: {i: {"status": "failed", "brands": []} for i in ids},
    )
    section = _compute(**BEFORE)["before_after"]
    assert section["status"] == "insufficient"
