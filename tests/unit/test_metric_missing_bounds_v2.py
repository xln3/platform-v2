...
def test_partial_api_failure_is_limited_and_separate_from_semantic_unknown() -> None:
    ...
    assert snapshot.state is MetricSnapshotState.LIMITED
    assert snapshot.value == Decimal(2) / Decimal(3)
    ...
    assert snapshot.unknown_answer_count == 0
    assert snapshot.failed_answer_count == 1
    assert "analysis_failed_partial" in snapshot.state_reason_codes
    assert "llm_api_rate_limited" in snapshot.state_reason_codes
...
def test_all_api_failures_make_snapshot_failed_not_semantically_unknown() -> None:
    definition = load_definitions().get("ai_recommendation_organic_mention_rate_v2", "2.0.0")
    request = SnapshotBuildRequest(
        definitions=(definition,),
        subjects=tuple(_subject(index, failed=True) for index in range(4)),
        as_of=datetime(2026, 8, 27, tzinfo=UTC),
        scope={"tenant": "ten_1", "project": "prj_1"},
        dependency_bundle={"answer_set_hash": "a" * 64},
        minimum_queries_for_ready=1,
    ...
    assert snapshot.known_answer_count == 0
    assert snapshot.unknown_answer_count == 0
    assert snapshot.failed_answer_count == 4
