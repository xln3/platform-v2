...
def project_domain(monkeypatch: pytest.MonkeyPatch) -> dict[str, str | None]:
    ...
    monkeypatch.setattr(
        brandrank_service, "fetch_project_brandrank_domain",
        lambda dsn, tenant, project: box["domain"],
    )
    # activities/s02.py 是直接 import 的函数引用，需同步 patch 模块内名字
    monkeypatch.setattr(
        s02_activities, "fetch_project_brandrank_domain",
        lambda dsn, tenant, project: box["domain"],
    )
    return box
...
class FakeClient:
    ...
    def _create(self, *, model, messages, temperature, response_format):
        ...
        return SimpleNamespace(choices=[SimpleNamespace(
            message=SimpleNamespace(content=json.dumps({"brands": outcome},
                                                       ensure_ascii=False)))])


async def test_happy_path_records_ok_row(recorded, project_domain,
...
async def test_idempotent_reextract_same_key(recorded, project_domain,
                                             monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_llm_failure_records_failed_row_without_exception_class(
        recorded, project_domain, monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_domain_unset_records_marker_without_llm(
    ...
    monkeypatch.setattr(extract, "default_client",
                        lambda: pytest.fail("domain 未设不应调 LLM"))
...
async def test_unknown_domain_column_value_fail_loud(
    ...
    monkeypatch.setenv("GEO_BRANDRANK_LLM_API_KEY", "k-test")
    monkeypatch.setattr(extract, "default_client",
                        lambda: pytest.fail("未知 domain 不应调 LLM"))

    result = await s02_activities.extract_brands_activity(PAYLOAD)

    assert result == {"state": "failed", "error": "unknown_domain",
                      "domain": "不存在的领域"}
    assert recorded[0]["status"] == "failed"
