...
Locator: getByText('详情投影已拒绝', { exact: true })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('详情投影已拒绝', { exact: true })
...
```ts
  ...
  190 |   await expect(
  191 |     page.getByText('当前检索窗口内的项目报告：服务返回 2 条，浏览器安全视图展示 1 条'),
  192 |   ).toBeVisible();
> 193 |   await expect(page.getByText('详情投影已拒绝', { exact: true })).toBeVisible();
  ...
  291 | });
  292 | 
  293 | test('numeric OTP and phone fields are rejected before structured report state', async ({
```