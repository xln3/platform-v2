...
GEO 品牌请求捕获受控 repository/release/SQLAlchemy 错误。调用方选择 `degrade` 时继续使用调用前已加载的 LKG entity master，并在 `knowledge_reasoning.degradation` 披露故障。选择 `fail` 时返回 503。程序错误不被宽泛吞掉。
...
同步器对坏 hash、重复 ID、断裂引用、跨 origin endpoint 和版本倒退 fail closed。失败不会推进 CURRENT。API 请求不应出现访问 Render 的连接或超时日志；若出现，视为架构回归。

## 升级回退

代码回退和知识回滚是两个动作。先回滚 active knowledge release，再回退应用版本。数据库 migration `s17_0001` 在存在知识历史后故意拒绝 downgrade；`s17_0002` 在存在 inference trace 时也拒绝删除 adoption/tool 血缘。恢复旧应用应保持 schema 向后兼容，不能删除审计历史。
