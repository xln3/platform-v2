...
| 2    | `outbound_disparagement_audit` | 找拉踩帖   | 全部 U 批次的冻结 v2 manifest |
...
- 服务须去重、升序且位于当前目录允许集。
- 新服务 5 被选中时，`sop_project_pub_id` 必填；服务 2 不再以 SOP/己方归属作为入池条件。
- 2026-08-24 前创建且带 SOP 项目的旧服务 2 请求保留 `own_content_v1` 兼容语义，重渲染时仍读取
  `formal-outbound-disparagement-v1`，不会把历史数字重解释成全 U。
...
新服务 2 只读取 `formal-service2-source-corpus-v2` 冻结 manifest。报告渲染不重新抓取页面、不调用
模型，也不从当前分页补算总数；U occurrence 与 distinct URL 分列。L1 事实性负面不计作拉踩，
B 暴露账不冒充逐字言论，publisher/commissioner 为 unknown 时不输出竞品委托或水军归因。
