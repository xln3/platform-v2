...
| W           | `content_contribution_analysis`、`weighted_content_chunk`、`weighted_content_chunk_review` | 正/负分析版本、页面/答案 span、哈希、分数、置信度、模型/提示词/策略/算法版本、追加式人工复核 |
...
管理信源接口位于 `/api/v2/internal/source-intelligence/projects/{project_pub_id}`，要求内部 intelligence 权限。固定钻取为项目→站点→URL→全部 occurrence→内部回答，并支持回答反向查看完整 UVW；URL 详情继续展示抓取尝试、页面版本、版本化体检、finding、逐字 span、W 历史版本和证据链。W 复核写入要求 `intelligence:review` 与幂等键；每次决定追加到 `weighted_content_chunk_review`，片段及 occurrence 上的状态只是可从最新分析和复核事实重建的当前投影。程序已拒绝的非精确 span 不能被人工改成有效 W。站点默认按 distinct URL、U occurrence、最近时间、host 排序；URL 默认按 U occurrence 排序；列表全部分页。
...
2. 服务 2 消费冻结范围内全部 U occurrence；发布方/委托方归属只影响独立 attribution，
   不影响入池。完整口径见 `SERVICE2_ALL_U_SOURCE_CORPUS.md`。
...
迁移 `s06_0037_uvw` 创建 UVW 事实表、RLS、索引、角色授权并兼容回填旧引用/旧 source document；`s06_0038_w_review` 增加 W 人工复核追加事实和复合租户/项目约束。升级顺序是数据库→source/analysis worker→API→前端。回滚前先停止新采集并导出新表；downgrade 会移除对应新表，但不改写旧回答与旧报告。

自动测试覆盖重复 URL occurrence、同 URL 多检索事件、重复 V/final 发生、750 个候选、历史小 limit 下完整抓取规划、V 不可观察、final 不自动成为 W、逐字 span 篡改拒绝、W/服务 5 历史重算版本、W 复核幂等与冲突、客户权限拒绝、管理双向钻取、三端导航和服务 5 两组对照。真实生产迁移、真实平台 U/V 网络证据、大规模抓取积压和生产人员复核操作仍需在部署阶段执行，不能由自动测试冒充已验证。
